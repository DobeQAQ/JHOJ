...
public class UserController {
    ...
    public String userListGet(HttpServletRequest request, ModelMap mp) {
        ...
        List<UserBean> userList = userService.getUserList(keyword, (page - 1) * Consts.COUNT_PER_PAGE, Consts.COUNT_PER_PAGE);
        int recordCount = userService.getCountOfSearch(keyword);
    ...
    public String userChartGet(HttpServletRequest request, ModelMap mp) {
        ...
        List<UserBean> userList = userService.getChart((page - 1) * Consts.COUNT_PER_PAGE, Consts.COUNT_PER_PAGE);
        int recordCount = userService.getCount();
        //获取分页信息
        PageBean pageBean = Utils.getPagination(recordCount, page, request);
    ...
    public String showUserGet(HttpServletRequest request, HttpServletResponse response, ModelMap mp) {
        ...
        if (strUserID != null) {
            ...
            List<Integer> acceptedProblem = viewSubmitRecordService.getUserAcceptedProblems(userID);
    ...
    public String editUserPost(HttpServletRequest request, HttpServletResponse response) {
        ...
        boolean userNameExist = userService.checkUserNameExist(userName) && !userName.equals(userBean.getUserName());
