...
public class DiscussController {
    ...
    private DiscussBean postDiscuss(HttpServletRequest request, HttpServletResponse response, boolean isOriginal) {
        ...
        if (type != Consts.FOR_MESSAGE) {
            ...
            if (type == 0) {
                discussBean.setTheme(problemService.getProblemByID(porcID).getTitle());
            } else {
                discussBean.setTheme(contestService.getContestByID(porcID).getTitle());
            }
        } else {
        ...
        if (discussBean.getRootID() == discussBean.getDirectFID() && discussBean.getRootID() == 0) {
            discussService.setAsRoot(discussBean);
        } else {
            discussService.updateReply(discussBean.getRootID());//对于回复消息, 将楼主的回复数量增加1
        }
    ...
    public String deleteDiscuss(HttpServletRequest request, HttpServletResponse response) {
        ...
        DiscussBean discussBean = discussService.getDiscussByPostID(postID);
        if (discussBean.getPostID() == discussBean.getRootID() && discussBean.getRootID() == discussBean.getDirectFID()) {
            ...
            discussService.deleteDiscussByPostID(postID);/*删除记录本身*/
            discussService.deleteDiscussByRootID(postID);/*删除该记录下的所有回复*/
            System.out.println("删除记录");
        } else {
            /*这是一条回复消息*/
            discussService.deleteDiscussByPostID(postID);/*删除记录本身*/

            discussService.updateReply(discussBean.getRootID());/*更新回复主题的回复数量*/
    ...
    public String discussListGet(HttpServletRequest request, ModelMap mp) {
        ...
        List<DiscussBean> discussList = discussService.getDiscussTitleList(type, porcID, theme, (page - 1) * Consts.COUNT_PER_PAGE, Consts.COUNT_PER_PAGE);
        int recordCount = discussService.getCountOfTitleList(type, porcID, theme);
    ...
    public String discussSetFirst(HttpServletRequest request) {
        ...
        if (postID != null) {
            if (val != null && val.equals("1")) {
                discussService.setFirst(postID, 1);
            } else {
                discussService.setFirst(postID, 0);
            }
        }
    ...
    public String getDiscussDetail(HttpServletRequest request, ModelMap mp) {
        ...
        Integer postID = strPostID != null && strPostID.length() > 0 ? Integer.parseInt(strPostID) : null;
        DiscussBean discussBean = discussService.getDiscussByPostID(postID);
        discussService.addWatch(postID);
        List<DiscussBean> replyList = discussService.getDiscussListByRootID(postID);
