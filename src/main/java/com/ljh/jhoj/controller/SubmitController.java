...
public class SubmitController {
    ...
    public String getSubmit(HttpSession session,
        ...
        //获取用户在之前对本题目的提交记录
        List<SubmitRecordBean> submitRecordBeans = submitRecordService.getSubmitRecordList(0, problemID, userID, null, null, 0, 3);
    ...
    public String postSubmit(HttpSession session,
        ...
        //提交数据库
        submitRecordService.addSubmitRecord(submitRecord);
    ...
    public String rejudgeGet(@RequestParam(value = "submitID", defaultValue = "-1") Integer submitID,
        ...
        /*获取提交信息*/
        SubmitRecordBean submitRecord = submitRecordService.getSubmitRecordByID(submitID);

        /*删除当前记录在数据库中的信息, 触发器会删除相关的judge-detail, compile-info*/
        submitRecordService.deleteSubmitRecord(submitID);
    ...
    public String getRecordList(HttpServletRequest request, ModelMap mp, HttpServletResponse response) {
        ...
        List<ViewSubmitRecordBean> submitRecordBeans = viewSubmitRecordService.getSubmitRecordList(0, problemID, userName, result, language, (page - 1) * Consts.COUNT_PER_PAGE, Consts.COUNT_PER_PAGE);

        //获取分页信息
        int recordCount = viewSubmitRecordService.getCountOnCondition(0, problemID, userName, result, language);
    ...
    public String getRecordDetail(HttpServletRequest request, ModelMap mp, HttpServletResponse response) {
        ...
        List<JudgeDetailBean> judgeDetailList = judgeDetailService.getJudegeDetailBySubmitID(submitID);

        SubmitRecordBean submitRecordBean = submitRecordService.getSubmitRecordByID(submitID);

        CompileInfoBean compileInfoBean = compileInfoService.getCompileResult(submitID);
        if (compileInfoBean == null) {
            compileInfoBean = new CompileInfoBean(submitID, "");
        }

        SystemErrorBean systemErrorBean = systemErrorService.getSystemErrorMessage(submitID);
